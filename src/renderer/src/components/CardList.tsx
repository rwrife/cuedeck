 ...
 * drag rows (by the grip) to reorder the running order — or, without a mouse,
 * focus a step and press Alt+↑ / Alt+↓ to move it (#39).
 *
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  onKeyDown={(e) => {
                    // Keyboard reordering (#39): Alt+ArrowUp/Down move the focused
                    // step, so the running order is fully operable without the
                    // mouse-only drag grip. Focus stays on this row (keyed by
                    // card.id) as it moves.
                    if (!isReorderKey(e)) return
                    const to = getReorderTargetIndex(e.key, i, deck.cards.length)
                    if (to === null) return
                    e.preventDefault()
                    reorderCards(i, to)
                  }}
                ...
                >
                  <span className="flex w-full items-center gap-2">
                    <span
                      ...
                      title="Drag to reorder (or focus the step and press Alt+↑ / Alt+↓)"
                      aria-hidden="true"
                    >
                      <GripIcon />
                    </span>
